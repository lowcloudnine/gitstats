...
Manages data collection from a git repository, preferably a centralized
repository.
...
class DataCollector:
    ...
    def collect(self, repo_dir):
        ...
        if len(conf.conf['project_name']) == 0:
            self.projectname = os.path.basename(os.path.abspath(repo_dir))
        else:
            self.projectname = conf.conf['project_name']
