...
The classes for creating the various reports/outputs that are the reason
gitstats exists.
...
class HTMLReportCreator(ReportCreator):
    def create(self, data, path):
        ...
        # copy static files. Looks in the binary directory, ../share/gitstats.py and /usr/share/gitstats.py
        binarypath = os.path.dirname(os.path.abspath(__file__))
        secondarypath = os.path.join(binarypath, '..', 'share', 'gitstats')
        basedirs = [binarypath, secondarypath, '/usr/share/gitstats']
        for file in (conf.conf['style'],
                     'sortable.js',
                     'arrow-up.gif',
                     'arrow-down.gif',
                     'arrow-none.gif'):
            for base in basedirs:
                ...
                print('Warning: "{}" not found, so not copied (searched: {})'
                      .format(file, basedirs))
        ...
        for d in range(0, 7):
            ...
            fp.write('%d %s %d\n' % (d + 1, conf.WEEKDAYS[d], commits))
            f.write('<tr>')
            f.write('<th>%s</th>' % (conf.WEEKDAYS[d]))
            if d in day_of_week:
        ...
        for weekday in range(0, 7):
            f.write('<tr><th>%s</th>' % (conf.WEEKDAYS[weekday]))
            for hour in range(0, 24):
                try:
        ...
        for author in data.getAuthors(conf.conf['max_authors']):
        ...
        if len(allauthors) > conf.conf['max_authors']:
            rest = allauthors[conf.conf['max_authors']:]
        ...
        if len(allauthors) > conf.conf['max_authors']:
            f.write('<p class="moreauthors">Only top %d authors shown</p>' % conf.conf['max_authors'])
        ...
        self.authors_to_plot = data.getAuthors(conf.conf['max_authors'])
            ...
            for author in self.authors_to_plot:
                if author in data.changes_by_date_by_author[stamp].keys():
                    lines_by_authors[author] = \
                        data.changes_by_date_by_author[stamp][author]['lines_added']
                    commits_by_authors[author] = \
                        data.changes_by_date_by_author[stamp][author]['commits']
        ...
        f.write('<tr><th>Month</th><th>Author</th><th>Commits (%%)</th><th class="unsortable">Next top %d</th><th>Number of authors</th></tr>' % conf.conf['authors_top'])
        for yymm in reversed(sorted(data.author_of_month.keys())):
            authordict = data.author_of_month[yymm]
            authors = getkeyssortedbyvalues(authordict)
            authors.reverse()
            commits = data.author_of_month[yymm][authors[0]]
            next = ', '.join(authors[1:conf.conf['authors_top']+1])
        ...
        f.write('<table class="sortable" id="aoy"><tr><th>Year</th><th>Author</th><th>Commits (%%)</th><th class="unsortable">Next top %d</th><th>Number of authors</th></tr>' % conf.conf['authors_top'])
    ...
    def createGraphs(self, path):
        ...
        for f in files:
            out = getpipeoutput([conf.gnuplot_cmd + ' "%s"' % f])
            if len(out) > 0:
                print
                out
    ...
    def printHeader(self, f, title=''):
        f.write(
            ...
            <body>
            """ % (self.title, conf.conf['style'], getversion()))
